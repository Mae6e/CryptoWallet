...
function sendBnbToken() {
  ...
  web3.eth.accounts.signTransaction(txObj, privKey, (errTx, signedTx) => {
    if (errTx) {
      console.log(JSON.stringify({status:'error', result:errTx}));
    } else {
      web3.eth.sendSignedTransaction(signedTx.rawTransaction, (err, res) => {
        if (err) {
          console.log(JSON.stringify({status:'error', result:err}));
        } else {
